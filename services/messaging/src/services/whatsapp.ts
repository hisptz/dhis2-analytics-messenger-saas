...
class WhatsappService {
				...
				try {
					...
					try {
						whitelistedPhoneNumbers =
							JSON.parse(
								`${process.env.ALLOWED_CONTACTS}` ?? "[]",
							) || [];
					} catch (e) {
						console.warn(
							`An invalid whitelisting configuration detected. Fix the environment variable. Ignoring the whitelist...`,
						);
					}

					const sanitizedPhoneNumber = from.number.replace(
						/@c.us|@g.us/,
						"",
					);
					if (
						!isEmpty(whitelistedPhoneNumbers) &&
						!whitelistedPhoneNumbers.includes(sanitizedPhoneNumber)
					) {
						console.info(
					...
					if (message.type.toLowerCase() !== "chat") {
						console.info(
							`Received message of type ${message.type} but only type chat is supported. Ignoring...`,
						);
	...
	private sanitizeReceivedMessage(
		...
		let sanitizedWhatsappPayload: WhatsappMessageResponse = {
			...
			message: {
				...
				file: ["document", "audio", "video"].includes(type)
					? body
					: null,
			},
			isForwarded,
	...
	private async sendDefaultErrorReplyMessage(
		destination: any,
	): Promise<void> {
