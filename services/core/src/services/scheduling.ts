// import { Schedule as AnalyticsPushJobSchedule } from "../apiSchemas/schedule";
//
...
// import { PushRequest } from "../apiSchemas/push";
//
// config();
//
...
// 		to: job.contacts.map((contact) => ({
// 			type: contact.type,
// 			number: contact.number,
// 		})) as any,
// 		description: job.description,
// 		visualizations: job.visualizations.map((visualization) => ({
// 			id: visualization.id,
...
// 	logger.info(
// 		`Sending push analytics ${data.visualizations
// 			.map(({ name }) => name)
// 			.join(", ")} To contacts ${data.to
// 			.map(({ number }) => number)
// 			.join(", ")}`,
// 	);
...
// 		const messages = await mapSeries(
// 			visualizations,
// 			asyncify(async (visualization: any) =>
// 				getMessage(visualization, {
// 					recipients: to,
// 					description,
// 					visualizerClient,
// 				}),
...
// 		);
//
// 		const messageResponse = await mapSeries(
// 			messages,
// 			asyncify(async (message: any) =>
// 				sendMessage(message, whatsappClient),
// 			),
// 		);
...
// 	data: AnalyticsPushJobSchedule & { job: AnalyticsPushJob },
// ) {
// 	const scheduledJobId = `${data.job.id}-${data.id}`;
// 	const isScheduleRunning = !!scheduledJobs.find(
// 		({ id }) => id === scheduledJobId,
// 	);
...
// 		const cronJobIndex = scheduledJobs.findIndex(
// 			({ id }) => id === scheduledJobId,
// 		);
